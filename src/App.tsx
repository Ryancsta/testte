    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden", background: `rgb(${bg.r}, ${bg.g}, ${bg.b})` }}>
      ...
      {modalOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "white", borderRadius: 20, padding: "32px 32px", boxShadow: "0 8px 40px rgba(0,0,0,0.22)", width: 380 }}>
      ...
      <button
        onClick={handleAlert}
        className="pulse-btn"
        style={{ position: "fixed", top: alertPos.top, left: alertPos.left, zIndex: 50, background: "transparent", border: "none", cursor: "pointer", padding: 0, lineHeight: 0, animationDelay: "3.1s" }}
      >
        <svg width="110" height="100" viewBox="0 0 100 90" xmlns="http://www.w3.org/2000/svg">
          ...
          <polygon points="50,6 94,84 6,84" fill="#c0392b" stroke="white" strokeWidth="4" strokeLinejoin="round" filter="url(#alertGlow)" />
          <text x="50" y="76" textAnchor="middle" fill="white" fontSize="52" fontWeight="bold" fontFamily="Arial, sans-serif" filter="url(#alertGlow)">!</text>
        </svg>
      </button>

      <div style={{ flex: 1, background: "transparent", transform: `rotate(${pageRotation}deg)`, transition: "transform 0.5s ease", position: "relative", display: "flex", flexDirection: "column" }}>
        ...
        <button onClick={rotatePage} style={{ ...neutralBtn, position: "absolute", top: 16, left: "50%", transform: "translateX(-50%)", zIndex: 1 }}>
          <RotateCw size={33} />
        </button>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 36 }}>
          ...
          <div style={{ transform: `rotate(${fieldsRotation}deg)`, transition: "transform 0.4s ease", display: "flex", flexDirection: "column", gap: 15, padding: 33, border: "1px solid #ddd", borderRadius: 15, background: "white", minWidth: 390 }}>
            <h1 style={{ fontFamily: FONTS[0], fontSize: 39, textAlign: "center", margin: 0, color: "#222" }}>Login</h1>

            <label style={{ fontFamily: FONTS[2], fontSize: 23, color: "#555" }}>Usuário</label>
            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} onKeyDown={handleKeyDown}
              style={{ padding: 17, fontSize: 24, fontFamily: FONTS[1], border: "1px solid #ccc", borderRadius: 12, outline: "none" }} />

            <label style={{ fontFamily: FONTS[3], fontSize: 23, color: "#555" }}>Senha</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown}
              style={{ padding: 17, fontSize: 24, fontFamily: FONTS[4], border: "1px solid #ccc", borderRadius: 12, outline: "none" }} />
            ...
            {message && (
              <div style={{ padding: 12, fontSize: 23, color: canAccess ? "green" : "#a00", textAlign: "center", fontFamily: FONTS[0] }}>
                {message}
              </div>
            )}
          </div>
        ...
        <button onClick={handleClear} className="pulse-btn" style={{ ...greenBtn, position: "absolute", bottom: 16, right: 16, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1 }}>
          <ArrowLeft size={36} />
        </button>
      </div>
